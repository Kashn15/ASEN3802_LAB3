...
for k = 1:length(AR_values)
    ...
    for j = 1:length(taper_ratios)
        ...
        [e_val, ~, ~] = PLLT(b_ref, 2*pi, 2*pi, c_t_val, c_r_val, 0, 0, 5, 5, N_terms); %Function call
...
% This function adds Prandtl Lifting Line Theory for finite wings
%
% Authors: Philip Austin, Charles Bailey, Nico Galindo, Natsumi Kakuda
% Date: 4/15/2026
%
...
%   c_r - chord at root (in feet)
%   aero_t - zero-lift AoA at tip (in degrees)
%   aero_r - zero-lift AoA at root (in degrees)
%   geo_t - geometric AoA at tip (in degrees)
%   geo_r - geometric AoA at root (in degrees)
...
% y coordinates from y = (b/2)*cos(theta)
%theta_i = i*pi/(2N) so theta goes from pi/(2N) to pi/2
% meaning we cover from near tip to root on one half the span position
position = cos(theta); % 0 at root (theta=pi/2), 1 at tip (theta is 0)

% Linear variation from root (eta=0) to tip (eta=1)
c_local    = c_r + (c_t - c_r) .* position;
a0_local   = a0_r + (a0_t - a0_r) .* position;
aero_local = aero_r + (aero_t - aero_r) .* position; % degrees not rad
geo_local  = geo_r  + (geo_t  - geo_r)  .* position; % degrees not rad

% Effective geometric angle of attack in radians
alpha_eff = deg2rad(geo_local - aero_local);

% Odd Fourier terms for n
