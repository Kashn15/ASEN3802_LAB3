...
% for PLLT.
% Aerospace Tool box is required to run this code.
%
...
Part = 3; % Select between Part 1 and Part 2 of Lab 3
...
elseif Part == 3
    Task1 = 1;
    Task2 = 1; % Task 1 must be = 1 for Task2 to operate
    Task3 = 1;
    Task4 = 1;
    Task5 = 1;
end
...
if Part == 3
    ...
    CL_NACA2412 = (CL_NACA2412_Pt2 - CL_NACA2412_Pt1) / (deg2rad(alpha_Pt2) - deg2rad(alpha_Pt1));
    ...
    CL_NACA0012 = (CL_NACA0012_Pt2 - CL_NACA0012_Pt1) / (deg2rad(alpha_Pt2) - deg2rad(alpha_Pt1));
    
    % Geometric Angles of Attack
    AOA_p3 = 4; % Degrees
    geo_t = AOA_p3 + 0; % degrees 4 + 0 = 4
    geo_r = AOA_p3 + 1; % degrees 4 + 1 = 5
        
    ALPHAS = linspace(-4,4,100);

    for i = 1:length(ALPHAS)
       [CL_NACA0012_cal(i)] = Vortex_Panel(x_0012, y_0012, V_inf, ALPHAS(i));
       [CL_NACA2412_cal(i)] = Vortex_Panel(x_2412, y_2412, V_inf, ALPHAS(i));
    end
    ...
    ix = find(d(1:end-1).*d(2:end) < 0);
    for k = 1:numel(ix)
         i = ix(k);
         t = d(i) / (d(i) - d(i+1)); % linear interpolation factor
         x_roots(end+1) = x(i) + t*(x(i+1)-x(i)); 
    end
            
    x_roots = unique(x_roots); % sort unique solutions
    % y_roots = zeros(size(x_roots)); % optional: get corresponding y (should be ≈0)
    
    % Aerodynamic AOA
    aero_r = x_roots; % NACA 2412
    aero_t = 0; % NACA 0012
    ...
    if Task1 == 1
        ...
        % Convergence Plot - Data Acquisition
        N_exact = 200; % 'Exact' solution we are assuming is a very large # of panels

        [e_exact, CL_exact, C_Di_exact] = PLLT(b, CL_NACA0012, CL_NACA2412, c_t, c_r, aero_t, aero_r, geo_t, geo_r, N_exact);
    
        fprintf('\nReference solution using N = %d odd terms:\n', N_exact);
        fprintf('CL_exact   = %.8f\n', CL_exact);
        fprintf('CDi_exact  = %.8f\n', C_Di_exact);
        fprintf('e_exact    = %.8f\n', e_exact);

        Nterms_VEC = 1:N_exact;
        CL_vals = zeros(size(Nterms_VEC));
        CDi_vals = zeros(size(Nterms_VEC));
        e_vals = zeros(size(Nterms_VEC));
    
        err_CL = zeros(size(Nterms_VEC));
        err_CDi = zeros(size(Nterms_VEC));
        
        for w = 1:length(Nterms_VEC)
            N_i = Nterms_VEC(w);

            [e_i, CL_i, CDi_i] = PLLT(b, CL_NACA0012, CL_NACA2412, c_t, c_r, aero_t, aero_r, geo_t, geo_r, N_i);

            e_vals(w) = e_i;
            CL_vals(w) = CL_i;
            CDi_vals(w) = CDi_i;
            err_CL(w) = abs((CL_i - CL_exact)/CL_exact) * 100;
            err_CDi(w) = abs((CDi_i - C_Di_exact)/C_Di_exact) * 100;

        end

        Thresholds = [10, 1, 0.1]; % percentage to meet errors

        Nreq_CL = zeros(size(Thresholds)); % Initialization
        Nreq_CDi = zeros(size(Thresholds)); % Initialization
    
        CLreq = zeros(size(Thresholds)); % Initialization
        CDireq = zeros(size(Thresholds)); % Initialization

        for e = 1:length(Thresholds)

            threshold = Thresholds(e);

            idxCL = find(err_CL <= threshold, 1, 'first');
            idxCDi = find(err_CDi <= threshold, 1, 'first');

            Nreq_CL(e) = Nterms_VEC(idxCL);
            Nreq_CDi(e) = Nterms_VEC(idxCDi);

            CLreq(e) = CL_vals(idxCL);
            CDireq(e) = CDi_vals(idxCDi);
        end
        
        %% Deliverable 1

        Deliverable1 = table(Thresholds', Nreq_CL(:), CLreq', Nreq_CDi(:) , CDireq','VariableNames', {'Relative Error Percentages','N_for_CL','CL_at_N','N_for_CDi','CDi_at_N'});
    
        fprintf('\n============================================================\n');
        fprintf('Deliverable 1 Table\n');
        fprintf('Wing at alpha = %.1f deg\n', AOA_p3);
        disp(Deliverable1);
    end

        %% Deliverable 2
    if Task2 == 1
        % Plot of Tolerances C_L
        figure(31);
        hold on;
        grid on;
        plot(Nterms_VEC, CL_vals, 'k-o', 'LineWidth', 1.2, 'MarkerSize', 2, 'MarkerEdgeColor', 'k', 'MarkerFaceColor', 'k', 'Displayname', 'C_L');
        yline(CL_exact, 'r-', 'LineWidth', 0.8, 'Displayname', 'Exact C_L');
        yline(CL_exact - (CL_exact*0.1), 'b--', 'LineWidth', 0.5, 'Displayname', '10% error');
        yline(CL_exact + (CL_exact*0.1), 'b--', 'LineWidth', 0.5, 'HandleVisibility', 'off');
        yline(CL_exact - (CL_exact*0.01), 'm--', 'LineWidth', 0.5, 'Displayname', '1% error');
        yline(CL_exact + (CL_exact*0.01), 'm--', 'LineWidth', 0.5, 'HandleVisibility', 'off');
        yline(CL_exact - (CL_exact*0.001), 'LineStyle', '--', 'Color', [88/255, 207/255, 70/255], 'LineWidth', 0.5, 'Displayname', '0.1% error');
        yline(CL_exact + (CL_exact*0.001), 'LineStyle', '--', 'Color', [88/255, 207/255, 70/255], 'LineWidth', 0.5, 'HandleVisibility', 'off');
        xlabel('Number of Odd Terms');
        ylabel('Lift Coefficient, C_L');
        title('Convergence of C_L with Number of Odd Terms');
        ylim([0.478, 0.54]);
        xlim([1, 25]);
        legend('Location', 'best');

        clr = {'b', 'm', [88/255, 207/255, 70/255]};

        for r = 1:length(Thresholds)
            iDx = find(err_CL <= Thresholds(r), 1,'first');
            
            x_pt = Nterms_VEC(iDx);
            y_pt = CL_vals(iDx);
            
            color = clr{r};
            xline(x_pt,'--', 'Color', color, 'LineWidth',1.2,'HandleVisibility','off');
            hold on;
            plot(x_pt, y_pt,'ko', 'MarkerSize', 8, 'MarkerFaceColor', color, 'MarkerEdgeColor', color, 'HandleVisibility','off');
            text(x_pt+0.5, y_pt+0.005, sprintf('%g%%\nN=%d\nC_L=%.4f', Thresholds(r), x_pt, y_pt),'FontSize',8,'BackgroundColor','white','EdgeColor', color, 'Margin', 4);
        end

        % Plot of Tolerances C_Di
        figure(32);
        hold on;
        grid on;
        plot(Nterms_VEC, CDi_vals, 'k-o', 'LineWidth', 1.2, 'MarkerSize', 2, 'MarkerEdgeColor', 'k', 'MarkerFaceColor', 'k', 'Displayname', 'C_D_i');
        yline(C_Di_exact, 'r-', 'LineWidth', 0.8, 'Displayname', 'Exact C_D_i');
        yline(C_Di_exact - (C_Di_exact*0.1), 'b--', 'LineWidth', 0.5, 'Displayname', '10% error');
        yline(C_Di_exact + (C_Di_exact*0.1), 'b--', 'LineWidth', 0.5, 'HandleVisibility', 'off');
        yline(C_Di_exact - (C_Di_exact*0.01), 'm--', 'LineWidth', 0.5, 'Displayname', '1% error');
        yline(C_Di_exact + (C_Di_exact*0.01), 'm--', 'LineWidth', 0.5, 'HandleVisibility', 'off');
        yline(C_Di_exact - (C_Di_exact*0.001), 'LineStyle', '--', 'Color', [88/255, 207/255, 70/255], 'LineWidth', 0.5, 'Displayname', '0.1% error');
        yline(C_Di_exact + (C_Di_exact*0.001), 'LineStyle', '--', 'Color', [88/255, 207/255, 70/255], 'LineWidth', 0.5, 'HandleVisibility', 'off');
        xlabel('Number of Odd Terms');
        ylabel('Induced Drag Coefficient, C_D_i');
        title('Convergence of C_D_i with Number of Odd Terms');
        ylim([0.01015, 0.0114]);
        xlim([2, 35]);
        legend('Location', 'best');
        ...
        for t = 1:length(Thresholds)
            iDX = find(err_CDi <= Thresholds(t), 1,'first');
            ...
            hold on;
            plot(x_PT, y_PT,'ko', 'MarkerSize', 8, 'MarkerFaceColor', color, 'MarkerEdgeColor', color, 'HandleVisibility','off');
            text(x_PT+0.45, y_PT+0.000095, sprintf('%g%%\nN=%d\nC_L=%.4f', Thresholds(r), x_PT, y_PT),'FontSize',8,'BackgroundColor','white','EdgeColor', color, 'Margin', 4);
        end
    end
    ...
    if Task3 == 1
        ...
        % Wing information
        Velo_knots = 100; % Knots
        Velo = Velo_knots * 0.514444; % [m/s]
        alt_ft = 10000; % [ft]
        alt = alt_ft / 3.281; % [m]
        
        [T, a, P, rho_inf] = atmosisa(alt);
        
        q_inf = 0.5 * rho_inf * Velo^2; % Dynamic Pressure
        
        S = 0.5 * (c_r + c_t) * b;

        % Node related to 0.1% relative error
        N_10th = max(Nreq_CL(3), Nreq_CDi(3));
        fprintf('\n============================================================\n');
        fprintf('\nMost restrictive number of odd terms for 0.1%% error = %d\n', N_10th);
        
        % PLLT: CL and C_Di Calculations
        [e_D3, CL_D3, CDi_D3] = PLLT(b, CL_NACA0012, CL_NACA2412, c_t, c_r, aero_t, aero_r, geo_t, geo_r, N_10th);

        % Lift/Drag Calculations
        L = q_inf * S * CL_D3; % [N]
        InducedDrag = q_inf * S * CDi_D3; % [N]

        % Digital data to find cd
        mu = 1.716e-5 * (T / 273.15)^(3/2) * ((273.15+S)/(T+S)); % Dynamic Viscosity [N*s/m^2] | T0 is Reference Temperature and mu0 is Reference viscosity)
        % nu = mu / rho_inf; % Kinematic Viscosity [m^2/s]
        Re = (rho_inf * Velo * c) / mu;
        fprintf('\n==========================================================\n');
        fprintf('\nReynolds Number = %d\n', Re);

        %Loading Experimental Data Cl vs Cd Plots
        data_0012 = readmatrix('NACA0012_ClvCd.csv');
        data_2412 = readmatrix('NACA2412_ClvCd.csv');
        ...
        [Cl_2412_sorted, idx2412] = sort(Cl_2412);
        [Cl_0012_sorted, idx0012] = sort(Cl_0012);
        Cd_2412_sorted = Cd_2412(idx2412);
        Cd_0012_sorted = Cd_0012(idx0012);

        [Cl_2412_unique, idx2412_unique] = unique(Cl_2412_sorted);
        Cd_2412_unique = Cd_2412_sorted(idx2412_unique);

        [Cl_0012_unique, idx0012_unique] = unique(Cl_0012_sorted);
        Cd_0012_unique = Cd_0012_sorted(idx0012_unique);

        cd_2412_op = interp1(Cl_2412_unique, Cd_2412_unique, CL_D3, 'linear', 'extrap');
        cd_0012_op = interp1(Cl_0012_unique, Cd_0012_unique, CL_D3, 'linear', 'extrap');

        cd_profile = mean([cd_2412_op, cd_0012_op]); % From Experimental 2D Data

        CD_D3 = cd_profile + CDi_D3;
        D_tot = q_inf * S * CD_D3; % [N]
        LD_ratio = L / D_tot;
        
        % Table
        Deliverable3 = table(N_10th, rho_inf, Velo, q_inf, S, CL_D3, CDi_D3, cd_profile, CD_D3, L, InducedDrag, D_tot, LD_ratio,'VariableNames', {'OddTermsUsed','rho_kg_m3','V_m_s','qInf_Pa','WingArea_m2','CL','CDi','cd_profile','CD_total','Lift_N','InducedDrag_N','TotalDrag_N','L_over_D'});
        fprintf('\n==========================================================\n');
        fprintf('Deliverable 3 Table\n');
        disp(Deliverable3);
...
%% Deliverable 4
    ...
    alpha_D4 = linspace(-4,12,100);
        
    CL_D4 = zeros(size(alpha_D4));
    CDi_D4 = zeros(size(alpha_D4));
    cd_profile_D4 = zeros(size(alpha_D4));
    CD_total_D4 = zeros(size(alpha_D4));
        
        for i = 1:length(alpha_D4)
        ...
        [e_i, CL_i, CDi_i] = PLLT(b, CL_NACA0012, CL_NACA2412, c_t, c_r, aero_t, aero_r, geo_t_i, geo_r_i, N_10th);
            
        CL_D4(i) = CL_i;
        CDi_D4(i) = CDi_i;
            
        cd_2412_i = interp1(Cl_2412_unique, Cd_2412_unique, CL_i, 'linear', 'extrap');
        cd_0012_i = interp1(Cl_0012_unique, Cd_0012_unique, CL_i, 'linear', 'extrap');
            
        cd_profile_D4(i) = mean([cd_2412_i, cd_0012_i]);
            
        CD_total_D4(i) = cd_profile_D4(i) + CDi_D4(i);
        ...
        hold on;
        grid on;
        plot(alpha_D4, CD_total_D4, 'k-', 'LineWidth', 1.5, 'DisplayName', 'C_D');
        plot(alpha_D4, cd_profile_D4, 'b-', 'LineWidth', 1.5, 'DisplayName', 'c_d');
        plot(alpha_D4, CDi_D4, 'r-', 'LineWidth', 1.5, 'DisplayName', 'C_{D_i}');
...
%% Deliverable 5
    if Task5 == 1
        ...
        LD_D5 = CL_D4 ./ CD_total_D4;
        
        figure(34);
        hold on;
        grid on;
        plot(alpha_D4, LD_D5, 'k-', 'LineWidth', 1.5, 'DisplayName', 'L/D');
        xlabel('Angle of Attack, \alpha (deg)');
...
% Span wise coordinates y = (b/2)*cos(theta)
